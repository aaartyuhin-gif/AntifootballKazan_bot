...
CREATE TABLE IF NOT EXISTS bookings (
    ...
    payment_id  TEXT,                       -- ЮKassa payment ID
    amount      REAL,                       -- сумма на момент записи
    paid_at     TEXT,
    created_at  TEXT DEFAULT (datetime('now','localtime')),
...
CREATE TABLE IF NOT EXISTS pricing_tiers (
    id          INTEGER PRIMARY KEY AUTOINCREMENT,
    min_players INTEGER NOT NULL,           -- от N игроков
    price       REAL NOT NULL               -- цена за игру с одного
);
...
INSERT OR IGNORE INTO settings VALUES ('club_name', '⚽ Футбольный клуб');
INSERT OR IGNORE INTO settings VALUES ('club_address', 'ул. Стадионная, 1');
...
# ── Users ─────────────────────────────────────────────────────────────────────
async def upsert_user(tg_id: int, username: str | None, full_name: str):
...
# ── Games ─────────────────────────────────────────────────────────────────────
async def create_game(date: str, time: str, max_players: int, field_name: str = "Главное поле", note: str = "") -> int:
    async with await get_db() as db:
...
async def get_upcoming_games() -> list[aiosqlite.Row]:
    async with await get_db() as db:
...
async def get_game(game_id: int) -> aiosqlite.Row | None:
    async with await get_db() as db:
...
async def get_games_needing_reminder() -> list[aiosqlite.Row]:
    """Игры, начинающиеся через 2 часа, которым ещё не отправили напоминание."""
    async with await get_db() as db:
        async with db.execute(
            """SELECT * FROM games
...
# ── Bookings ──────────────────────────────────────────────────────────────────
async def get_game_bookings(game_id: int) -> list[aiosqlite.Row]:
    async with await get_db() as db:
        async with db.execute(
...
async def count_active_bookings(game_id: int) -> int:
    async with await get_db() as db:
        async with db.execute(
...
async def get_user_booking(game_id: int, user_id: int) -> aiosqlite.Row | None:
    async with await get_db() as db:
        async with db.execute(
...
async def create_booking(game_id: int, user_id: int, amount: float) -> int:
    async with await get_db() as db:
        cur = await db.execute(
...
async def cancel_booking(game_id: int, user_id: int):
    async with await get_db() as db:
        await db.execute(
...
async def set_payment_id(game_id: int, user_id: int, payment_id: str):
    async with await get_db() as db:
        await db.execute(
...
async def mark_paid_by_payment_id(payment_id: str):
    async with await get_db() as db:
        await db.execute(
...
async def get_user_bookings(user_id: int) -> list[aiosqlite.Row]:
    async with await get_db() as db:
        async with db.execute(
...
# ── Pricing ───────────────────────────────────────────────────────────────────
async def get_pricing_tiers() -> list[aiosqlite.Row]:
    async with await get_db() as db:
        async with db.execute("SELECT * FROM pricing_tiers ORDER BY min_players") as cur:
            return await cur.fetchall()


async def set_pricing_tiers(tiers: list[tuple[int, float]]):
    """Полная замена таблицы тарифов."""
    async with await get_db() as db:
...
def calc_price(player_count: int, tiers: list) -> float:
    """Возвращает цену для текущего количества игроков."""
...
def next_tier(player_count: int, tiers: list) -> tuple[int, float] | None:
    """Следующий порог снижения цены."""
    for tier in sorted(tiers, key=lambda t: t["min_players"]):
